...
- `app` composes boarding deposit policy into booking-triage evaluation in `retired clean-slate contract: `app/tests/workflow_service_composition_contracts.rs``. The test explicitly keeps `booking_triage` from owning the boarding deposit decision: it maps `domain::boarding::deposit::ConfirmationReadiness` into app-level readiness and approval gates.
...
- `storage::operations::CoreServiceContractsRecord` stores a boarding contract with the other core service contracts in [`storage/src/operations.rs`](../../../storage/src/operations.rs). Storage coverage in `retired clean-slate contract: `storage/tests/core_service_contract_storage.rs`` round-trips `domain::boarding::Contract::standard_petsuites` and rejects invalid validated boarding scalars such as zero room inventory.
