...
pub struct CoreServiceContracts {
    pub location_id: LocationId,
    pub boarding: boarding::Contract,
    pub daycare: daycare::Contract,
            ...
            pub fn readiness_for_confirmation(
                ...
                match (
                    ...
                    (
                        ...
                        _,
                        Some(
                            payment::DepositStatus::Paid | payment::DepositStatus::WaivedByManager,
                        ),
...
pub mod daycare {
    ...
    pub mod eligibility {
        ...
        impl GroupPlayPolicy {
            pub fn evaluate(&self, evidence: &Evidence) -> GroupPlayDecision {
                if let incident::Restriction::SuspendedPendingManagerReview { pet_id } =
                    evidence.incident
                {
    ...
    pub mod attendance {
        ...
        impl DateRange {
            ...
            ) -> std::result::Result<Self, DateRangeError> {
                if end < start {
        ...
        impl AttendanceDays {
            ...
            ) -> std::result::Result<Self, AttendanceDaysError> {
                if days.is_empty() {
            ...
            pub const fn lane(&self) -> QueueLane {
                match self.decision {
                    ...
                    ReadinessDecision::BlockedForSafetyOrPolicy { .. } => {
                        QueueLane::WaitlistOrPolicyLane
                    }
                }
            }
        }
            ...
            pub fn evaluate(&self, context: &ReadinessContext) -> ReadinessDecision {
                ...
                match &context.eligibility {
                    ...
                    EligibilityReadiness::GroupPlay(
                        eligibility::GroupPlayDecision::NeedsStaffReview { gate, .. },
                    ...
                    EligibilityReadiness::GroupPlay(
                        eligibility::GroupPlayDecision::Ineligible { .. },
                    ) => ReadinessDecision::BlockedForSafetyOrPolicy {
                        gate: policy::ReviewGate::ManagerApproval,
                    },
                }
