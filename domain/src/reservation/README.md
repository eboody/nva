...
- [`app::checkout_completion`](../../../app/src/checkout_completion.rs) consumes `domain::entities::reservation::Id` plus reported source-status and handoff evidence. Current evaluation and fail-closed hydration leave the suggested reservation status absent; serialized labels cannot suggest or authorize `CheckedOut`.
