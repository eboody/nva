...
- `GET /readyz` via `readyz`: makes stubbed dependencies explicit (`database`, `object_storage`, fake deterministic `agent_runtime`, disabled live customer/provider writes) and labels the local observability proof scope versus production gaps. See [`tests/health_contract.rs`](./tests/health_contract.rs).
...
- `GET /agent/context/data-quality-hygiene` via `data_quality_hygiene_agent_context`: returns source-grounded internal cleanup candidates, blocked side effects, review gates, and labor estimates without repairing source records.
- `POST /agent/drafts/data-quality-hygiene` via `submit_data_quality_hygiene_agent_draft`: validates internal cleanup recommendations, source refs, issue refs, review gates, and blocked side effects before staff sees them as reviewable work.
- `POST /data-quality-hygiene/actions/{action_id}/outcome` via `capture_data_quality_hygiene_action_outcome`: captures reviewed cleanup disposition, source/issue refs, resolution status, labor-minute evidence, and local correlation/outbox-candidate observability as a `storage::operations::DataQualityHygieneOutcomeRecord` plus review-gated local projection in state.
- `GET /data-quality-hygiene/outcomes/summary` via `data_quality_hygiene_outcome_summary`: reports a location/day/correlation rollup from reviewed local outcomes, preserving source refs, issue refs, disposition counts, and labor-minute totals while keeping live side effects disabled.
- `GET /ops/metrics/summary` via `ops_metrics_summary`: reports local in-memory aggregate counters for contract-demo proof, including manager brief outcomes and Data-Quality Hygiene reviewed/pending outbox candidates; it explicitly labels the production queue adapter as `not_configured`.
...
Data-quality hygiene follows the same adapter pattern for source cleanup labor. [`data_quality_hygiene_agent_context`](./src/http.rs) exposes source refs, issue refs, affected entities, review gates, blocked actions, and labor impact for internal cleanup review. [`submit_data_quality_hygiene_agent_draft`](./src/http.rs) rejects provider/PMS mutation, customer messages, schedule/payment movement, ambiguity hiding, and missing source/issue evidence. [`capture_data_quality_hygiene_action_outcome`](./src/http.rs) records reviewed cleanup feedback as `storage::operations::DataQualityHygieneOutcomeRecord`, returns local observability fields that correlate the request/workflow/review/outbox candidate, and creates only a review-gated internal outbox projection with `live_delivery_allowed: false`. [`data_quality_hygiene_outcome_summary`](./src/http.rs) rolls those local outcomes up by location, operating day, and optional correlation id. [`ops_metrics_summary`](./src/http.rs) exposes demo-only aggregate counters for Data-Quality Hygiene outbox candidates and review-gated pending candidates while declaring `production_queue_adapter: "not_configured"`. [`tests/data_quality_hygiene_agent_contract.rs`](./tests/data_quality_hygiene_agent_contract.rs) proves the loop remains review-only and reports labor minutes without claiming that a Gingr/PMS/source record was repaired or any live queue delivered.
...
| Data-quality context query | private `DataQualityHygieneAgentContextQuery` | [`src/http.rs`](./src/http.rs) | Query shape for location/operating-day cleanup packets. |
| Data-quality draft submission | private `DataQualityHygieneAgentDraftSubmissionRequest` | [`src/http.rs`](./src/http.rs) | HTTP boundary for agent-proposed internal cleanup actions. |
| Data-quality submitted action | private `DataQualityHygieneSubmittedAction` | [`src/http.rs`](./src/http.rs) | Draft action payload validated for source refs, issue refs, review gates, and blocked side effects. |
| Data-quality outcome request | private `DataQualityHygieneOutcomeCaptureRequest` | [`src/http.rs`](./src/http.rs) | HTTP boundary for reviewed cleanup feedback and actual labor-minute capture. |
| Data-quality outcome summary query | private `DataQualityHygieneOutcomeSummaryQuery` | [`src/http.rs`](./src/http.rs) | Query shape for location/day/correlation labor proof rollups. |
| Data-quality outcome projections | `storage::operations::{DataQualityHygieneOutcomeRecord, DataQualityHygieneOutcomeSummary}` | [`../../storage/src/operations.rs`](../../storage/src/operations.rs) | Persistable/storage-shaped cleanup-labor evidence and read-only summary reporting created from API-local outcomes. |
| Data-quality app workflow | `app::data_quality_hygiene::{Request, Workflow, Packet, Action, OutcomeRecord}` | [`../../app/src/data_quality_hygiene.rs`](../../app/src/data_quality_hygiene.rs) | Source-grounded cleanup workflow consumed by context, draft, outcome, and summary handlers. |
