...
- `POST /data-quality-hygiene/actions/{action_id}/outcome` via `capture_data_quality_hygiene_action_outcome`: captures reviewed cleanup disposition, source/issue refs, resolution status, and labor-minute evidence as a `storage::operations::DataQualityHygieneOutcomeRecord` in local state.
