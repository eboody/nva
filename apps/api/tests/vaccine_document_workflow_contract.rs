...
fn test_auth_router() -> axum::Router {
    static STATE: OnceLock<http::VaccineDocumentState> = OnceLock::new();
    http::router_with_test_auth_state(
        STATE
            .get_or_init(http::VaccineDocumentState::default)
            .clone(),
    )
}
...
async fn json_request(
    ...
    let actor_id = payload["uploaded_by_staff_id"]
        .as_str()
        .or_else(|| payload["reviewed_by_staff_id"].as_str())
        .map(str::to_owned);
    let role = if payload["reviewed_by_staff_id"].is_string() {
        "medical_reviewer"
    } else {
        "front_desk_lead"
    };
