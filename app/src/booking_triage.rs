...
impl AiRecommendation {
    /// Builds or derives new data for the booking-readiness workflow contract.
    pub const fn new(
        recommended_action: AgentRecommendedAction,
        rationale: RecommendationText,
    ) -> Self {
...
pub mod reservation {
    ...
    /// Defines the behavior required from a repository participant in the booking triage workflow.
    pub trait Repository {
        /// Runs the id step while preserving the booking-readiness workflow safety boundary.
    ...
    R: reservation::Repository,
{
    ...
    /// Returns the evaluate carried by this booking-readiness workflow value.
