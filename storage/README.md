...
- [`src/operations.rs`](./src/operations.rs) owns the cross-service storage records: `PetResortPortfolioRecord`, `ServiceOfferingRecord`, `CoreServiceContractsRecord`, `TechnologyEcosystemRecord`, and `ManagerDailyBriefOutcomeRecord`. It also owns `Error`, `CodecError`, `RecordKind`, `ShapeMismatchReason`, and `StorageField`.
