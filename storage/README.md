...
| --- | --- | --- | --- |
| Crate surface | `storage` | [`src/lib.rs`](./src/lib.rs) | Exposes `storage::operations` and `storage::service_line`; re-exports `CodecError` and `Result`. |
...
| Shape/error classifiers | `storage::operations::{ShapeMismatchReason, StorageField}` | [`src/operations.rs`](./src/operations.rs) | Identify malformed persisted shapes and specific invalid fields. |
...
4. Validated storage scalars and shape mismatch errors turn bad source data into typed failures (`StorageField`, `ShapeMismatchReason`) instead of quiet fallthrough or ad hoc manual investigation.
...
- Executable storage contracts live in [`storage/tests`](./tests): `retired clean-slate contract: `operations_storage_contracts.rs``, `retired clean-slate contract: `core_service_contract_storage.rs``, `retired clean-slate contract: `manager_daily_brief_outcome_storage.rs``, `retired clean-slate contract: `data_quality_hygiene_outcome_storage.rs``, `retired clean-slate contract: `data_quality_read_model_storage.rs``, and [`mvp_migration_contract.rs`](./tests/mvp_migration_contract.rs).

## Data-Quality Hygiene persistence/read-model proof

The first owned API persistence proof is intentionally narrow and safe:

- [`../migrations/0001_mvp_foundation.sql`](../migrations/0001_mvp_foundation.sql) defines the complete current schema, including `source_import_runs`, `source_quality_issues`, `sync_gaps`, and the BI-safe views `source_quality_backlog`, `data_quality_hygiene_labor_outcomes`, `audit_lineage`, and `import_freshness`.
