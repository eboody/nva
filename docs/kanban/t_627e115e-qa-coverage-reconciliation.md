...
| API / worker / CLI runtime shell | PASS WITH LOCALIZED DOC FIXES | Runtime/storage lane found API, worker, CLI, and app tool surfaces are understandable as local/test/stubbed or read-only shells, not live side-effect authority. | Follow-up: worker crate-root Rustdoc overclaims “durable Postgres-backed”; change wording to safe local/background shell or future-adapter language. Optional public runtime-shell evidence links. |
