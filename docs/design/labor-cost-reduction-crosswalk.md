...
4. Confirm the proof status: source evidence and provenance first, then domain/app contract, then review-gated packet or draft, then reviewed outcome/labor minutes if that storage proof exists.
...
- Workflow authority lives in app/domain contracts that build packets, validate drafts, name blocked actions, and record reviewed outcomes.
...
- Storage/runtime/reporting proof can show reviewed local outcomes or local contract behavior, but local/demo proof must not be promoted into production NVA labor-savings claims.
...
| Sales, lead conversion, retention, and grooming rebooking | Leads/forms/calls, completed stays, grooming history, package/membership history, contact consent, campaign source, reservations. | Customer/pet opportunity projection with source-grounded reason code, consent, due cadence, slot utilization, and suppression/eligibility status. | `domain::lead`, `app::crm_retention`, `domain::grooming::{rebooking, appointment}`, `domain::retail::recommendation`, `app::manager_daily_brief::BriefActionKind::ApproveRetentionFollowUpDraft`. | [Grooming rebooking / retention](../workflows/operator/grooming-rebooking-retention.md) queue that prioritizes safe, consented follow-up candidates and records staff disposition. | Agent drafts personalized follow-up language for review. No live message, discount, payment movement, or provider update without app-owned approval. | Reported queue-review and draft-writing time remains nonclaimable evidence; revenue lift is a separate future measurement. Use the retention row in [workflow packets](../entity-atlas/contract-crosswalk/workflow-packets.md#workflow-by-workflow-entity-map) and note the current storage gap for durable retention outcome evidence. |
