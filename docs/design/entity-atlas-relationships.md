...
| Human decision / reviewed outcome | is recorded | Outcome record, audit event, approval record, actual labor minutes | [Outcome/labor atlas](entity-atlas-outcomes-operations-money.md), `domain/src/entities.rs`, canonical `domain/src/audit.rs`, `app/src/*::OutcomeRecord`, `storage/src/operations.rs` | Reviewed outcome evidence remains nonclaimable and does not establish realized labor savings. |
