...
- Edges marked `docs-only conceptual / Deferred proof item` are useful explanatory links that need a future contract before they can be treated as implementation-backed.
...
flowchart LR
    ...
    F -->|caller-reported disposition; app outcome evidence contracts| G[Outcome record + reported minute labels]
...
| Domain entity, source refs, and policy context | compose an app request | Workflow packet or suppressed evidence packet | [Workflow packets atlas](entity-atlas-workflow-packets-agents.md), `app/src/booking_triage.rs`, `app/src/data_quality_hygiene.rs`, `app/src/checkout_completion.rs`, `app/src/crm_retention.rs`, `app/src/daily_update.rs`, `app/src/manager_daily_brief.rs` | App workflow owns its bundle, not live PMS/customer/payment authority; CRM Retention remains ineligible and creates no review queue. |
| Workflow packet | may be summarized/ranked/drafted only where its explicit capability permits | Agent prompt packet, customer-message draft, internal task draft, audit-event draft, or suppression-only evidence | `app/src/agents.rs`, `domain/src/agent.rs`, `domain/src/workflow.rs`, family page [workflow packets](entity-atlas-workflow-packets-agents.md) | CRM Retention cannot rank, queue, task, or draft; other agent output is draft-only unless separate opaque action authority exists. |
...
| Caller-reported disposition | is retained | Outcome record, audit-event reference, approval-record reference, reported time fields | [Outcome/labor atlas](entity-atlas-outcomes-operations-money.md), `domain/src/entities.rs`, canonical `domain/src/audit.rs`, `app/src/*::OutcomeRecord`, `storage/src/operations.rs` | Outcome evidence remains nonclaimable and does not authenticate a decision, prove review/completion, or establish realized labor savings. |
| Outcome record and source refs | are persisted/projected | Storage records, reporting evidence, API/local inspection | [Runtime/storage/API atlas](entity-atlas-runtime-storage-api-surfaces.md), `storage/src/operations.rs`, storage tests, API tests | Storage proves only that reported evidence was retained; it does not prove what was reviewed or measured and cannot authorize future live actions. |
...
| Provider surface gap marker | records unsupported or undocumented provider surface | Data-quality issue or Deferred proof item mapping gap | `integrations/gingr/src/dto/mod.rs`, [Gingr provider boundary atlas](../integrations/gingr/provider-boundary-atlas.md), [Source/provenance atlas](source-provenance-data-quality-atlas.md) | Explicitly docs/source-backed as a gap; no inferred DTO semantics. |
...
| --- | --- | --- | --- | --- |
| Reservation and reservation status | [Booking Triage](../workflows/operator/booking-triage.md), [Checkout Completion](../workflows/operator/checkout-completion.md), [Daily Updates](../workflows/operator/daily-updates-pawgress-drafts.md), [Grooming Rebooking / Retention](../workflows/operator/grooming-rebooking-retention.md) | [PetSuites core entities](entity-atlas-petsuites-core-entities.md), `domain/src/reservation/mod.rs`, `domain/src/entities.rs`, `app/src/booking_triage.rs`, `app/src/checkout_completion.rs` | Draft readiness/checkout evidence summaries and supported internal tasks; retention receives only suppressed, ineligible reservation evidence and no task or draft. | No booking confirmation/rejection, PMS status write, check-in/out, schedule/capacity mutation, or promotion of reservation history into retention authority. |
| Customer and contact channel | Booking Triage, Daily Updates, Retention, Data Quality Hygiene | [PetSuites core entities](entity-atlas-petsuites-core-entities.md), `domain/src/customer.rs`, `domain/src/entities.rs`, `app/src/crm_retention.rs`, `app/src/daily_update.rs` | Booking and Daily Updates may draft customer-safe scripts or messages under their own approval contracts; retention only preserves suppressed, ineligible contact evidence and cannot draft. | No autonomous customer/member or marketing send, and no promotion of serialized contact evidence into retention authority. |
...
| Grooming contract and rebooking cadence | [Grooming Rebooking / Retention](../workflows/operator/grooming-rebooking-retention.md), Manager Daily Brief | [Revenue opportunity entities](entity-atlas-revenue-opportunity-entities.md), `domain/src/grooming/mod.rs`, `app/src/crm_retention.rs`, `storage/src/service_line/grooming.rs` | Preserve cadence and reported opportunity evidence in a suppressed, ineligible packet; current runtime creates no candidate, ranking, queue item, task, or draft. | No appointment booking, customer send, discount/credit, service promise, or promotion of serialized evidence into retention authority. |
...
| Reported labor-evidence rollup | No authenticated performer is established; manager/regional ops may inspect the nonclaimable evidence | reported estimate differences, reported minute labels, action kind, caller-provided persona/reporting group, source refs, disposition, outcome record id | [Outcome/labor atlas](entity-atlas-outcomes-operations-money.md), `app/src/manager_daily_brief.rs`, `app/src/data_quality_hygiene.rs`, `storage/src/operations.rs` |
...
| Review gates/blocked actions | Policy, workflow-local blocked actions, human roles | Approval records, caller-reported dispositions, audit events | [Review gates atlas](entity-atlas-review-safety-boundaries.md), `domain/src/policy.rs`, `domain/src/workflow.rs` | Safety story must stay visible on every workflow page. |
| Outcomes/labor/operations/money | Reviewed workflow action, reported labor feedback, source refs | Storage records, reports, readiness evidence | [Outcome/labor atlas](entity-atlas-outcomes-operations-money.md), `app/src/manager_daily_brief.rs`, `app/src/data_quality_hygiene.rs`, `storage/src/operations.rs` | Retains reported, nonclaimable evidence; it does not convert a hypothesis into realized-value proof. |
...
## Conceptual/Deferred proof item edges to carry forward
...
| --- | --- | --- |
| Verified Gingr webhook event -> every downstream workflow packet | Event-driven workflows would reduce polling/reconciliation labor. | Partly backed for webhook verification by `integrations/gingr/src/webhook.rs`; downstream workflow-specific mapping remains docs-only conceptual / Deferred proof item unless a workflow test/source path names it. |
...
| Regional labor exception rollup -> staffing or scheduling action | Regional operators need labor-risk visibility. | Manager/regional summary is supported as draft/reporting evidence; direct staffing/payroll/schedule changes remain blocked and docs-only conceptual / Deferred proof item for any future approval workflow. |
| Service-line record projections -> public non-coder service-line dashboards | Dashboards could help managers review readiness by line. | Storage/domain service-line records exist; dashboard/page implementation is docs-only conceptual / Deferred proof item unless linked to runtime/API routes. |
...
5. Does every outcome/time rollup remain explicitly nonclaimable rather than being presented as realized labor-savings authority?
6. Are diagrams and adjacency tables source-backed, or explicitly marked `docs-only conceptual / Deferred proof item`?
