...
- Prior caller-reported outcome records when available, including source refs, issue refs, actor/persona/disposition/time labels, wrong-source labels, and correlation ids; these are nonclaimable and do not prove review, completion, measured labor, or value. See [`../../../storage/src/operations.rs`](../../../storage/src/operations.rs).
...
| Source-data quality issue | Rank repair queues, flag blocking/critical issues, recommend “do not rely on this source fact,” and retain caller-reported issue-disposition/wrong-source labels without asserting review or source adjudication. | `domain::data_quality::{Issue, Severity, ResolutionStatus}`, storage data-quality hygiene outcome records. |
...
| Caller-reported labor/value evidence | Minute labels, reported time or estimate differences, wrong-source findings, reported rework or handle-time labels, feedback, reporting group, correlation id. | A caller report was retained for inspection. | Review, completed work, measured labor or value, guaranteed ROI, or safe future automation without review. |
