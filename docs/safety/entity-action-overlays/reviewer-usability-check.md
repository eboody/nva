...
| [`template.md`](template.md) | Enforces the entity/action-first section order. | Requires each claim to cite source/Rustdoc/tests or be listed as a gap. | Provides the default blocked-action and reviewer-role structure. | Requires outcome/audit fields and an explicit nonclaimable labor-evidence posture. | Pass as future overlay authoring template. |
...
| Actual ROI is not established by documentation or caller-reported outcome rows. | Treat before/actual-minute, wrong-source, disposition, and correlation fields as nonclaimable reports. | Separately authenticated review plus authoritative labor/value measurement, not prose estimates or serializable outcome labels. |
