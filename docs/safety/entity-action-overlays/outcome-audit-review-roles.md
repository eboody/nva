...
This overlay helps front-desk leads, managers, medical/vaccine reviewers, behavior/daycare leads, customer-message reviewers, payment/accounting reviewers, IT/security, and product/ops owners reduce repeated “who signs off and what proves it?” handoffs by separating source evidence, draft/recommendation work, human approval, any live action elsewhere, and durable nonclaimable outcome evidence.

Use this page as reusable language for sibling entity/action overlays. It is not a generic policy page and it does not approve new live automation. It tells writers how to route an entity/action to the right reviewer, retain outcome/audit evidence, and identify the separate authority needed to prove safe use.
...
4. Which outcome/audit record retains the reported disposition and time spent without claiming realized savings?

The labor problem is repeated manager/front-desk reconciliation after an agent prepares work: staff have to re-check source records, decide whether a manager/medical/payment/message/security reviewer is required, and then later report their disposition and time spent. The safe outcome is a role-owned, source-backed workflow packet plus a durable nonclaimable outcome record with reviewer, disposition, correlation, source, blocked-action, and minute evidence.
...
| Manager Daily Brief | Ranks supported manager/front-desk work and retains caller-reported feedback and time fields without claiming review, completion, measured labor, or changing live systems. | Operating day, source facts, checkout packets, suppressed retention evidence, data-quality issues, brief action, outcome record. | `../../../app/src/manager_daily_brief.rs`; `../../../storage/src/operations.rs`; `../../design/manager-daily-brief-measurable-labor-loop.md`. |
| Data-quality hygiene | Turns duplicate/missing/stale/ambiguous source facts into reviewable cleanup-task drafts and caller-reported outcome evidence without proving review, hiding ambiguity, or repairing provider records autonomously. | Source refs, provenance, issue refs, source record, customer/pet/reservation/service fields, cleanup task, outcome record. | `../../../domain/src/source.rs`; `../../../domain/src/data_quality.rs`; `../../../storage/src/operations.rs`; `../source-evidence-map.md`. |
| Daily updates / supported customer messaging | Prepares customer-visible drafts, contact/channel checks, and approval artifacts only where the cited workflow issues draft authority. | Message, draft body ref, customer/pet/reservation/stay facts, contact permission, included/omitted facts, approval record. | `../../../app/src/agents.rs`; `../../../app/src/tools.rs`; `../../../domain/src/message.rs`; `../../design/labor-cost-reduction-crosswalk.md`. |
| CRM retention | Preserves universally ineligible/suppressed source and outcome evidence only. | Caller-reported retention labels and source refs. | `../../../app/src/crm_retention.rs`; `../../../storage/src/operations.rs`. Current code creates no candidate, ranking, queue, task, review packet, draft, contact, conversion, completion, or value authority. |
...
| Storage outcome records | Persist caller-reported, nonclaimable disposition and time evidence; they neither authenticate review nor authorize live provider, customer-message, payment, or schedule actions. | Action id, disposition, before/reported minutes, caller actor/persona labels, source refs, correlation id, reporting dimensions. | `../../../storage/src/operations.rs`; `../../../app/src/manager_daily_brief.rs`; `../source-evidence-map.md`. |
...
| Outcome/audit record | App/storage outcome record, workflow event/result, API validation result, tool-port draft/failure record. | For caller-created outcome rows, only that reported labels/time fields were retained; separately issued audit or approval evidence may prove its own exact event. | Actor identity, review, completion, contact, measured minutes/value, guaranteed ROI, or live action unless separate authoritative evidence proves that exact fact. |
...
- Existing outcome evidence: prior caller-reported dispositions, time fields, wrong-source labels, issue refs, and status labels. These may inform later review but do not authenticate or measure prior work.
...
| Rank work | Manager daily brief demand/capacity/checkout/data-quality actions and regional exception queue when implemented. Current retention evidence cannot create or enter a queue. | Ranking saves scanning time in supported workflows; it does not change schedules, provider records, or priorities in source systems. |
...
| Draft customer-message text | Confirmation, missing-info, daily-update, or supported public-response draft and fact checklist. Current retention evidence cannot create a customer draft. | Customer-message reviewer or approved sender must approve recipient, channel, timing, body, contact permission, and suppression in workflows that currently issue draft authority. |
...
| Record reported outcome | Outcome record with caller-reported disposition, actor/reviewer/persona labels, reported minutes spent, source refs, issue refs, feedback, correlation id. | Preserves nonclaimable history; it does not authenticate review or prove completion, conversion, labor savings, realized value, or live action. |
...
| --- | --- | --- | --- |
| Staff/front-desk lead | Routine intake completeness, source-backed supported queue work, handoff quality, checkout review, and inspection of reported retention evidence. Current retention packets create no queue or internal task. | Retention eligibility/queue/task/draft authority, manager exceptions, money movement, medical/vaccine/behavior approval, provider write-back permission, IT/security scope. | Booking triage missing-info packet; checkout exception task; ineligible retention evidence. Evidence: staff actor, packet/action id, source refs, and non-authoritative disposition. |
...
| Customer-message reviewer / approved sender | Final recipient, channel, timing, body, contact preference/consent, included/omitted facts, suppression/escalation, send/no-send decision for supported message workflows. Current retention evidence never reaches this lane. | Retention eligibility/task/draft authority, provider/PMS mutation, payment movement, medical/behavior approval, schedule/capacity changes. | Daily update/Pawgress draft or booking missing-info message. Retention supplies only suppressed, ineligible evidence with no draft id or send disposition authority. |
...
| Durable outcome/value record | Caller-reported disposition, feedback, reported time fields or estimate differences, actor/persona labels, source refs, data-quality findings, correlation id, reporting group. | What the caller reported for later review. | That review or work happened, measured labor value, guaranteed ROI, revenue lift, or future savings. |
...
For labor evidence, use caller-report phrasing: “this outcome record retained a 12-minute caller report against a 45-minute caller-reported baseline” or “the loop retains a caller-reported time difference without proving review or measurement.” Do not say “the agent saved ROI” or “automation reduced labor cost”: serialized outcome records do not authenticate review, measure labor, or issue realized-value authority, even in aggregate. Reported estimates cannot by themselves authorize queue work; reported minutes and dispositions remain nonclaimable evidence.
...
| Demand-versus-staffing manager brief | Operating-day service-demand facts, labor/schedule source facts, source refs and data-quality flags. | Ranked manager action with labor estimate. | Manager/general manager reviews schedule/capacity/staffing implication. | `OutcomeRecord` / `ManagerDailyBriefOutcomeRecord`: action id, caller-reported actor/persona/disposition/time labels, source refs, correlation id; this row does not prove review or labor effect. |
| Retention or grooming evidence packet | Reported stay/grooming history, contact-permission history, consent/channel claims, source refs, and suppression reasons. | Ineligible evidence packet only; current serialized inputs create no candidate, queue item, internal task, customer draft, contact, conversion, completion, or value authority. | Staff may inspect evidence; no current reviewer can promote the serialized packet into executable authority. | Source refs, reported disposition, suppression evidence, and explicit absence of payment/discount/schedule/provider mutation or claimed queue/draft labor value. |
| Source-data duplicate or stale vaccine issue | Source refs/provenance, duplicate candidates or stale evidence, data-quality issue severity/kind. | Internal cleanup task or data-quality hygiene draft. | Staff/front-desk lead for routine cleanup; manager for high-impact source ambiguity; medical reviewer for vaccine validity; IT/product for provider write-back mode. | Issue refs and blocked-action proof; caller-reported outcome actor/persona/disposition/time labels remain nonclaimable and do not prove review or repair. |
...
- `../../../storage/src/operations.rs` for `ManagerDailyBriefOutcomeRecord`, `DataQualityHygieneOutcomeRecord`, reported before/actual minutes, actor/persona, source refs, correlation id, and nonclaimable reported estimate differences.
