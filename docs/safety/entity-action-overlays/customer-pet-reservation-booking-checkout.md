...
- Retention staff manually inspect reported post-stay, consent, and source evidence; the current executable contract does not establish safe follow-up opportunities.
...
| Retention follow-up | Preserves reported checkout, contact-permission, opportunity, consent/channel, and suppression evidence for human inspection. Current serialized inputs cannot prepare follow-up drafts, suppression tasks, queues, or internal work. | Customer, reservation, checkout packet, contact permission, opportunity evidence, source refs, message channel, staff review packet, outcome record. | [Grooming Rebooking / Retention operator page](../../workflows/operator/grooming-rebooking-retention.md); [app/src/crm_retention.rs](../../../app/src/crm_retention.rs). |
| Manager daily brief | Ranks source-grounded manager actions such as demand/staffing review, checkout exceptions, and data-quality issues while blocking side effects. Reported retention packets cannot add queue work or a draft action. | Location, operating day, service-demand facts, checkout packets, reported retention packets, manager brief actions, labor estimates, outcome records. | [app/src/manager_daily_brief.rs](../../../app/src/manager_daily_brief.rs); [storage/src/operations.rs](../../../storage/src/operations.rs). |
...
| Checkout packet | [app/src/checkout_completion.rs](../../../app/src/checkout_completion.rs) `Request`, `Packet`, `CompletionStatus`, `StaffHandoff`, `SafeAgentAction`, `BlockedAction`, and `AuditEventDraft`. | Whether serialized evidence routes to manager review or source reconciliation; it never supports a checkout-completion suggestion. | Checkout readiness/completion, retention eligibility/drafts, live PMS checkout mutation, payment/refund/discount movement, capacity release, or customer message send. |
| Retention packet/outcome | [app/src/crm_retention.rs](../../../app/src/crm_retention.rs) `ContactPermission`, `FollowUpEligibility`, `StaffReviewPacket`, `Packet`, `SafeAgentAction`, `BlockedAction`, and `OutcomeRecord`. | Current ineligibility/suppression evidence, manager-review gates, source refs, blocked actions, and staff-reported outcome shape. | Eligibility, draft/queue/task creation, customer outreach, booking/rebooking creation, discount application, payment movement, provider/PMS/calendar mutation, or consent override. |
| Manager daily brief/action evidence | [app/src/manager_daily_brief.rs](../../../app/src/manager_daily_brief.rs) and [storage/src/operations.rs](../../../storage/src/operations.rs). | Ranked supported source-grounded manager actions, estimates, and caller-reported time/disposition evidence where stored. The reported fields do not prove review, completion, labor effect, or value. | Staff schedule changes, customer messages, provider/PMS writes, money movement, hidden data-quality cleanup, or guaranteed ROI. |
...
| Report checkout evidence | `CompletionStatus::ReportedStaffCheckout` preserves a serialized staff label while manager review, checkout execution, capacity release, billing closeout, and messaging remain blocked. | Evidence/review only; it cannot suggest or authorize checked-out status. |
| Preserve reported retention evidence | `crm_retention::StaffReviewPacket` and `Packet` remain suppressed evidence. | Serialized checkout/consent evidence cannot create eligibility, ranking, queue work, an internal task, or a customer draft. `DraftCustomerFollowUpForReview` is a compatibility label, not an available action. |
| Record retention outcome evidence | `crm_retention::OutcomeRecord` with reservation/customer ids, caller-provided actor/timestamp/outcome labels, provenance, and opportunity evidence. | Retains nonclaimable caller-reported history only; it proves no identity, review, contact, booking, conversion, completion, or value. |
...
| Customer-message reviewer/approved sender | Final recipient, channel, timing, body, consent/contact preference, suppression, and sensitive wording for supported booking or checkout drafts. Current retention evidence creates no draft-review path. Maps to `CustomerMessageApproval` only where an executable workflow issues it. | Retention eligibility/drafting, provider/PMS mutation, payment movement, medical/behavior/vaccine approval, or capacity changes. |
...
| Check-in/out or checkout-completion review | Observed source reservation status, source provenance, reservation id, staff handoff actor/time, belongings status, care summary, departure-note review, and review gates. | Always route serialized checkout evidence to manager review or source reconciliation; never suggest checked-out status from packet agreement. |
| Retention follow-up draft | Serialized checkout, customer/reservation, contact-permission, channel, consent, opportunity, and provenance records remain evidence only. | Mark ineligible and route to manager review; serialized checkout evidence cannot unlock a draft, queue, internal task, or customer contact. |
...
| Checkout evidence packet | `checkout_completion::Packet`, `CompletionStatus`, absent suggested status, required review gates, `AuditEventDraft`, and blocked actions. | Reported source checkout and staff-handoff labels were classified for review. | `CheckedOut` was suggested or authorized; live PMS checkout/closeout, billing action, capacity release, or customer send happened. |
| Retention outcome evidence | `crm_retention::OutcomeRecord` with reservation id, customer id, caller-reported actor/timestamp, `FollowUpOutcome`, source provenance, and opportunity evidence. | Nonclaimable correlation history was retained. | Identity, review, contact, conversion, completion, follow-up value, booking, or discount authority. |
| Manager daily-brief reported labor evidence | `manager_daily_brief::OutcomeRecord` and storage manager daily-brief outcome records with caller-reported before/actual-minute fields, reported difference, disposition, actor/persona labels, and source refs. | Nonclaimable grouped history was retained. | Identity, review, completion, measured labor, labor effect, realized value, guaranteed savings, or permission for schedule/provider/customer/payment side effects. |
| Data-quality reported evidence | Storage data-quality outcome records and source issue refs where used. | Caller-reported wrong-source/source-quality disposition and timing labels were retained. | Identity, review, repair completion, measured cleanup value, or authority to delete or overwrite source records. |
...
Explicit value-measurement fields for this family should include: minutes avoided finding/reading source records; booking handle time; checkout audit time; rework reduced; wrong-source findings; missing/stale/conflicting source counts; staff/manager disposition; suppressed/deferred/approved/send-by-human counts; actual minutes; before/after minutes; reported time difference; feedback; reporting group; source refs; issue refs; packet/action/draft/outcome/correlation ids.

Current evidence supports app-level booking/checkout packets, suppressed retention evidence, caller-reported manager daily-brief outcome records, and storage source refs. Dedicated durable booking-triage and checkout-completion outcome projections are not identified, and caller-created outcome rows cannot establish production labor value without separate authoritative measurement.
...
- [domain/src/entities.rs](../../../domain/src/entities.rs) for customer, pet, reservation, service, add-on, hard-stop, care/temperament, status/source, and actor/approval entities; [domain/src/audit.rs](../../../domain/src/audit.rs) is the one canonical owner for audit events, actions, subjects, and validated extension metadata.
...
| Dedicated durable checkout-completion outcome projection not identified in storage | Checkout audit savings and closeout value need durable outcome capture. | Use checkout packet/audit-event drafts for local proof; do not claim production checkout labor savings. | Storage/API contract for checkout outcome, handoff disposition, reported time spent or estimate differences, source refs, reviewer, and blocked side effects. |
...
| Retention outcome persistence is caller-reported compatibility evidence | Current retention is universally ineligible and cannot establish follow-up value. | Preserve only suppressed/nonclaimable history; do not use it for candidates, queues, tasks, drafts, contact, conversion, completion, or value metrics. | A future path would require new opaque non-serializable eligibility/action authority plus separately authoritative measurement. |
