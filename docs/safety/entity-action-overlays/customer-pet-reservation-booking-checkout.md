...
| Draft retention follow-up | `crm_retention::SafeAgentAction::DraftCustomerFollowUpForReview` remains modeled but is unavailable from serialized checkout/consent evidence. | A future opaque authenticated marketing/contact authority would be required; no send, discount, rebooking, or provider write is available. |
...
| Manager daily-brief labor value | `manager_daily_brief::OutcomeRecord` and storage manager daily-brief outcome records with before/actual minutes, reported time difference, outcome, recorded_by, source refs. | Reviewed manager action and measured labor minutes for that action where implemented. | Guaranteed future savings or permission for schedule/provider/customer/payment side effects. |
