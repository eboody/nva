...
| Record reported outcome evidence | `crm_retention::OutcomeRecord`, `manager_daily_brief::OutcomeRecord`, and storage manager daily brief outcome records. | Reported disposition, actor/reviewer, source refs/provenance, before minutes, actual minutes spent, or feedback must exist; no realized-savings claim is issued. |
...
| Manager brief outcome evidence | `manager_daily_brief::OutcomeRecord` and `storage::operations::ManagerDailyBriefOutcomeRecord`: action id, actor/persona, outcome, before minutes, actual minutes spent, source refs, reporting group. | Reported manager/front-desk work evidence only. | Realized labor value, production ROI, staffing changes, customer sends, source cleanup, provider writes, or future savings. |
