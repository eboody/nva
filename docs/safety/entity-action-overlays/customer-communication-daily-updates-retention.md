...
- Retention and grooming rebooking follow-up: serialized stay, checkout, grooming, consent, and opportunity records remain suppressed or manager-review evidence; they cannot establish eligibility or unlock a draft.
...
| CRM retention and grooming rebooking | Builds suppressed or manager-review evidence packets; current serialized checkout, opportunity, and contact records cannot establish eligibility or support a draft. | Checkout packet, customer, reservation, grooming/service-history evidence, contact permission, source refs, review gates, outcome record. | `../../../app/src/crm_retention.rs`; `../../../app/tests/crm_retention_workflow_contracts.rs`; `../../workflows/operator/grooming-rebooking-retention.md`. |
| Checkout completion alongside retention | Supplies reported checkout context only; it cannot establish retention eligibility, unlock review work, or authorize a follow-up draft. | Reservation, source checkout/PMS status, staff handoff, belongings, care summary, departure notes, audit-event drafts. | `../../../app/src/checkout_completion.rs`; `../../../app/tests/checkout_completion_workflow_contracts.rs`; `../../workflows/operator/checkout-completion.md`. |
...
| Outcome/labor evidence | Stores or shapes caller-reported, nonclaimable outcome evidence where supported. Manager brief has durable storage projection; retention has an app outcome record; Daily Updates and checkout currently identify outcome gaps. | Caller-reported actor/reviewer labels, disposition, before/after or actual-minutes fields, source refs, reporting group, and blocked-action reasons. These fields do not prove reviewer identity, completion, labor effect, or value. | `../../../storage/src/operations.rs`; `../../../app/src/manager_daily_brief.rs`; `../../../app/src/crm_retention.rs`. |
...
| Retention/grooming follow-up packet | `app/src/crm_retention.rs` owns reported contact, opportunity, suppression, source-ref, and outcome evidence. | Current serialized inputs remain ineligible and suppressed; no candidate queue, internal task, customer draft, or customer-message review route is available. | Booking creation, autonomous outreach, discount/payment action, calendar mutation, or dedicated durable grooming-retention storage. |
| Checkout completion packet | `app/src/checkout_completion.rs` owns source checkout observation, staff handoff evidence, checkout classification, safe actions, blocked actions, and audit-event drafts. | Whether reported checkout context needs manager review or source reconciliation; it cannot feed retention eligibility, queue, task, or draft authority. | Final checkout execution, retention authority, capacity release, billing closeout, provider/PMS write, or customer send. |
| Manager brief packet and outcome | `app/src/manager_daily_brief.rs` and `storage/src/operations.rs` own ranked actions, safe/blocked actions, reported before/actual-minute fields, and durable manager brief outcome projection. | The server-issued packet proves which supported actions were presented. The caller-created outcome proves only that reported labels and fields were retained; it does not prove reviewer identity, work completion, measured minutes, or labor effect. | Production ROI, realized value, staffing/schedule changes, source cleanup, customer sends, or money movement. |
...
| Preserve reported retention or grooming evidence | `crm_retention::StaffReviewPacket` and `Packet`. | Current serialized checkout and consent evidence remains suppressed or manager-review evidence and cannot create a candidate, ranking, queue item, task, or draft. |
| Create supported internal staff review task | Checkout/manager-brief safe actions or `tools::draft_update`/task-draft equivalents. | Internal-task-only for supported checkout handoff, source-ambiguity, or data-quality paths. Current retention evidence cannot create an internal task. |
| Rank manager/front-desk queue | `manager_daily_brief::BriefAction` and packet `actions`. | Actions must be source-grounded; manager brief ranks demand-staffing review, checkout exceptions, capacity recommendations, and data-quality investigations without live side effects. Reported retention packets cannot produce an action, queue item, task, or draft. |
...
| Record reported outcome evidence | `crm_retention::OutcomeRecord`, `manager_daily_brief::OutcomeRecord`, and storage manager daily brief outcome records. | Caller-reported disposition, actor/reviewer labels, source refs/provenance, before-minute fields, actual-minutes-spent fields, or feedback may be retained as nonclaimable history; no identity, completion, measured-time, or realized-savings claim is issued. |
...
| Treat estimated minutes or draft creation as realized labor savings. | Caller-reported disposition and minute labels remain nonclaimable and prove no review, action, measurement, completion, or value. | Retain outcome/feedback/reported minute labels while explicitly preserving the value-authority gap. |
| Treat `crm_retention::SafeAgentAction::DraftCustomerFollowUpForReview` as an available capability. | The enum label is retained for compatibility, but every current serialized retention packet is ineligible and the workflow cannot emit the action. | Preserve the packet as suppressed evidence. A future opaque authenticated authority would require a new executable contract and customer-message approval. |
...
| Front-desk lead | Checkout handoff quality, unresolved handoff routing, and inspection of reported retention evidence. Current retention packets cannot create queue work or internal tasks. | Final customer-send authority unless assigned, retention eligibility/task/draft authority, payments/refunds, medical/behavior approval, broad source cleanup, automation policy. |
| Customer-message reviewer or approved sender | Final recipient, channel, timing, body, tone, consent/contact preference, suppression, and approved send path for supported daily-update drafts. Current retention evidence produces no draft-review route. Maps to `CustomerMessageApproval` only where an executable workflow issues it. | Provider/PMS writes, retention eligibility/drafting, payment movement, medical/behavior/vaccine approval, staffing or capacity changes. |
...
| Grooming manager or retention operator | Grooming cadence/rebooking context, service-history interpretation, and inspection of reported retention evidence. Current serialized inputs cannot establish an opportunity, prioritization, queue, task, or draft. | Eligibility or queue authority, live booking/calendar mutation, discounts/payment, final customer sends, DNC/consent override. |
...
| Retention/grooming follow-up draft | Current serialized checkout, consent, opportunity, and provenance records are evidence only and cannot establish contact eligibility. | Keep the packet ineligible/suppressed with `ManagerApproval`; no draft or outreach is available without a future opaque authenticated contact authority. |
...
| Outcome/value record | Caller-reported actor/reviewer labels, disposition, source refs/provenance, outcome code, timestamp, and before/after or actual-minute fields where supported. | Treat every field as nonclaimable history; do not infer reviewer identity, completion, measured labor effect, or realized savings. |
...
| Retention outcome/value | `crm_retention::OutcomeRecord` fields: reservation id, customer id, recorded-by label, recorded-at field, `FollowUpOutcome`, source provenance, evidence; `records_staff_evidence_only()`. | A caller-provided follow-up disposition label was retained as evidence, including labels such as booked-next-stay, interested, not-interested, no-response, or suppressed. It does not prove staff identity, contact, conversion, booking, completion, or value. | Dedicated durable production persistence, authenticated staff action, or autonomous outreach. |
| Manager brief outcome evidence | `manager_daily_brief::OutcomeRecord` and `storage::operations::ManagerDailyBriefOutcomeRecord`: action id, actor/persona labels, outcome, before-minute field, actual-minutes-spent field, source refs, reporting group. | Caller-reported manager/front-desk history only; durable retention does not promote it into trusted identity, completion, or measured labor evidence. | Realized labor value, production ROI, staffing changes, customer sends, source cleanup, provider writes, or future savings. |
...
Nonclaimable labor-evidence rules for this family:

- Reported minute evidence: retain manager-brief before/after estimates and reported actual time spent without deriving minutes avoided; Daily Updates and checkout remain estimated/planned even after an outcome record lands.
- Rework evidence: count omitted facts, internal flags, source-data-quality issues, wrong-source findings, suppressed outreach, and manager/staff dispositions without labeling the count as reduced rework.
- Handle-time evidence: retain caller-reported packet time and caller-reported prior manual queue-scanning time as separate observations; serialized outcomes do not establish reduced handle time.
- Customer-trust evidence: record approval/suppression/disposition and blocked-send evidence; no collection of these serialized outcomes establishes ROI or realized value.
