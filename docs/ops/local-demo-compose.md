...
4. Waits for API `/v1/healthz`, API `/v1/readyz`, and the staff-web root page.
5. Calls `POST /v1/demo/information-lifespan/run` and `GET /v1/demo/information-lifespan/{correlation_id}/report`.
...
- `pet-resort-api` on `127.0.0.1:${PET_RESORT_API_HOST_PORT:-3001}` with `.var/information-lifespan` mounted read-only so `/v1/demo/information-lifespan/*` can show the processor artifact
