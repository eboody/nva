...
| Reviewed disposition and labor impact | Routes every serialized checkout packet through manager review and retains reported work-duration evidence. | Reviewable app packet evidence only; it cannot prove checkout completion or realized savings. | Source `app/src/checkout_completion.rs` (`ReviewedDisposition`, `LaborImpact`, `LaborMinutes`); domain `domain/src/reservation/mod.rs` (`CheckoutCompletionDisposition`, `OutcomeRecord`, `OutcomeState`). |
