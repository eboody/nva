...
Status: supported app/API/storage contract and local smoke evidence. The workflow ranks, drafts, validates, and records caller-reported cleanup labels as nonclaimable evidence; it does not automatically repair Gingr/PMS records, merge profiles, approve vaccines, change payments, or hide source ambiguity.
...
| Outcome record and labor minutes | Retains caller-reported disposition, issue refs, source refs, before-minute estimates, reported actual minutes, owner-persona label, and resolution-status label. These fields do not prove review or completion. | App outcome and storage projection are nonclaimable evidence; they prove neither provider repair, authenticated review, measured labor, nor realized savings. | [`app/src/data_quality_hygiene.rs`](../../../app/src/data_quality_hygiene.rs) (`OutcomeRecord`, `FeedbackOutcome`); [`storage/src/operations.rs`](../../../storage/src/operations.rs) (`DataQualityHygieneOutcomeRecord`); focused outcome tests. |
...
| --- | --- | --- | --- |
| `app` | `app::data_quality_hygiene::{Request, Packet, Candidate, Action, DraftSubmission, DraftValidation, SafeAgentAction, BlockedAction, OutcomeRecord, Workflow}` | Source-grounded packet building, ranked hygiene actions, internal cleanup drafts, draft validation, labor estimates, and admission of caller-reported outcome evidence. | Proof of review, completion, measured labor/value, live provider/PMS repair, customer sends, payment/refund/discount movement, schedule changes, source hiding, destructive merges/deletes, or sensitive payload release. |
...
| `storage` | `storage::operations::DataQualityHygieneOutcomeRecord`, `DataQualityHygieneOutcomeSummary`, outcome/persona/action/resolution codes, stored source refs, labor-minute scalar | Durable evidence of reported dispositions, actual time spent, estimate differences, and source/issue refs supporting a location/day/correlation rollup. | Proof that the provider record changed, external work completed, or production labor savings were realized. |
| `apps/api` | `/agent/context/data-quality-hygiene`, `/agent/drafts/data-quality-hygiene`, `/data-quality-hygiene/actions/{action_id}/outcome`, `/data-quality-hygiene/outcomes/summary` contract tests | Local/API shell evidence for context, draft rejection, caller-reported outcome capture, and reported-outcome summary without live side effects. | Authenticated review/completion, measured labor/value, production connector behavior, live writeback, or unrestricted API authorization claims. |
...
- Estimate before/after reconciliation minutes and retain caller-reported disposition/time labels as nonclaimable history; admission checks do not prove the reported review or work occurred.
...
- Estimated labor value: the packet can compare before/after reconciliation minutes for the candidate queue. Local API contract evidence shows a sample context with 55 before minutes, 22 after minutes, and 33 reported estimated minute difference.
- Reported outcome evidence: `OutcomeRecord` and `DataQualityHygieneOutcomeRecord` capture action id, outcome, before minutes, reported actual minutes spent, actor/persona, feedback, source refs, issue refs, location, operating day, correlation id, action kind, owner persona, and reported resolution status. `DataQualityHygieneOutcomeSummary` aggregates reported evidence without deriving reported actual minutes spent.
- Current evidence status: supported app/API/storage contract and local smoke/spec evidence. Storage codecs preserve reported time/provenance fields, reject zero-minute evidence, and API tests prove the summary route reports caller-provided minutes without enabling live side effects or authenticating review.
- Caveat: outcome capture retains caller-reported cleanup disposition and time evidence; it does not authenticate review, prove labor effect or source repair, establish production savings, or show that a live provider writeback exists.
