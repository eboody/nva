...
The workflow keeps the suspect fact, field path, provenance, source record reference, severity, freshness, sensitivity, and workflow-blocking status visible. That gives staff a precise queue: what looks wrong, which source record produced it, why it blocks or degrades another workflow, and who must review the cleanup.
...
It needs [data-quality issue](../../glossary-source-data-terms.md#domaindata_qualityissue-data-quality-issue) candidates, [source refs](../../glossary-architecture-terms.md#source-ref-domainsourcerecordref), [provenance](../../glossary-architecture-terms.md#provenance-domainsourceprovenance), location, operating day, entity/field path, severity, resolution status, source freshness, sensitivity/redaction metadata, workflow-blocking state, BI visibility, issue refs, action refs, and labor-minute estimates.
...
| Hygiene candidate/action | Groups issue evidence into staff-reviewable work and assigns action kind, priority, owner persona, review gates, and labor estimate. | `app::data_quality_hygiene` derives the review packet from source-grounded candidates; staff owns the cleanup decision. | [`app/src/data_quality_hygiene.rs`](../../../app/src/data_quality_hygiene.rs) (`Candidate`, `Action`, `ActionKind`, `ActionPriority`, `HygienePersona`, `RemovedManualWork`, `LaborImpactEstimate`). |
...
| `storage` | `storage::operations::DataQualityHygieneOutcomeRecord`, outcome/persona/action/resolution codes, stored source refs, labor-minute scalar | Durable evidence that a reviewed cleanup action was completed/deferred/suppressed/source-wrong/not-actionable and how many minutes were saved or spent. | Proof that the underlying provider record was changed or that production labor savings were independently audited. |
| `apps/api` | `/agent/context/data-quality-hygiene`, `/agent/drafts/data-quality-hygiene`, `/data-quality-hygiene/actions/{action_id}/outcome` contract tests | Local/API shell evidence for context, draft rejection, and outcome capture without live side effects. | Production connector behavior, live writeback, or unrestricted API authorization claims. |
...
- Group candidates by issue, field path, severity, freshness, sensitivity, workflow-blocking status, owner persona, and manual work removed.
...
- Source: [`storage/src/operations.rs`](../../../storage/src/operations.rs) (`storage::operations::{DataQualityHygieneOutcomeRecord, DataQualityHygieneReportingGroup, DataQualityHygieneOutcomeCode, DataQualityResolutionStatusCode, StoredSourceRecordRef}`).
- Source/API shell: [`apps/api/src/http.rs`](../../../apps/api/src/http.rs) for context, draft-validation, and outcome routes.
